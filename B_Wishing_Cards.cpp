...
void solve() {
    int n, k;
        ...
        for (int i = 0; i < n; i++) {
            cin >> a[i];
            a[i] = min(a[i], k); // no need to exceed k
        }

        // dp[j][x]: max happiness using j cards, current max = x
        vector<vector<long long>> dp(k + 1, vector<long long>(k + 1, NEG));
        dp[0][0] = 0;

        for (int i = 0; i < n; i++) {
            vector<vector<long long>> ndp(k + 1, vector<long long>(k + 1, NEG));

            for (int used = 0; used <= k; used++) {
                for (int mx = 0; mx <= k; mx++) {
                    if (dp[used][mx] < 0) continue;

                    // give c cards to friend i
                    for (int c = 0; c <= a[i] && used + c <= k; c++) {
                        int new_mx = max(mx, c);
                        ndp[used + c][new_mx] = max(
                            ndp[used + c][new_mx],
                            dp[used][mx] + new_mx
                        );
                    }
