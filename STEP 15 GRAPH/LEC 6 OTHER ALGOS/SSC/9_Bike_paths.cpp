...
void m_conq() {
    ...
    for(int i = 0; i < m; i++){
        int u, v;
        cin >> u >> v;
        adj[u].push_back(v);  // for scc
        edge.push_back({u, v}); // for dp
    }
    ...
    for(auto &i : edge){  // adj for u and v such that u and v does not belong so same scc.
        int u = i.first;
        int v = i.second;
        if(root[u] != root[v]) adj[u].push_back(v);
    }

    memset(dp, -1, sizeof(dp));
    for(int u = 1; u <= n; u++){
        if(root[u] == u && dp[root[u]] == -1) dfs1(root[u]);
    }

    for(int u = 1; u <= n; u++){
        cout << dp[root[u]] - 1 << endl;
    }
}
