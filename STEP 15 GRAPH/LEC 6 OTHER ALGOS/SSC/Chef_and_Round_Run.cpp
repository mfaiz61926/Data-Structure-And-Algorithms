...
void m_conq() {
        ...
        //resetting globals for multi test environment.
        timer = 0;
        scc.clear();
        for(int i = 1; i <= n; i++){
            tin[i] = low[i] = inStack[i] = 0;
            adj[i].clear();
        }
        ...
        for(int i = 1; i <= n; i++){
            int x;
            cin >> x;
            x += 1;  // x means jumps, if x == 0, means one jump to adjacent val, so x = 1;
            x %= n;
            nextBox[i] = i + x;
            if(nextBox[i] > n) nextBox[i] %= n;

            ans += nextBox[i] == i;  // counting self loops
            if(nextBox[i] != i){  // and for rest of nodes build scc.
                adj[i].push_back(nextBox[i]);
            }
        }
        ...
        for(auto &v : scc){
            ans += (v.size() * (v.size() > 1));  //in one scc, you start with any node comes back to same node.
        }
