...
void m_conq() {
    ...
    for(int i = 0; i < n; i++){
        ...
        cin >> a >> c >> d >> b;
        string s = "";
        for(int j = 0; j < a.size() - 1; j++) s += a[j];
        a = s;
    ...
    sort(all(com));
    com.erase(unique(all(com)), com.end()); // give each name a unique index;
    int m = com.size(); // total node => m
    vector<pair<int,int>>edge; // then those indexes forms edge x->y

    for(auto &it : e){
        string a = it.first;
        string b = it.second;
        int x, y;
        x = lower_bound(all(com), a) - com.begin() + 1;  //finding index 1 based.
        y = lower_bound(all(com), b) - com.begin() + 1;
        edge.push_back({x, y});
        adj[x].push_back(y);
    }

    for(int i = 1; i <= m; i++) if(!tin[i]) dfs(i); // for scc
    for(auto &it : edge){
        int u = it.first, v = it.second;
        if(root[u] != root[v]) in[root[u]]++;  //in of root[u]++
    }
    int cnt = 0;
    for(int i = 1; i <= m; i++) cnt += (root[i] == i && in[i] == 0);
    cout << (cnt > 1 ? "impossible" : "possible") << endl;
