...
void m_conq() {
    ...
    int v = u;
    for(int i = 0; i < k; i++){
        if(!is_ancestor(nodes[i], u)){
            if(v == u || depth[nodes[i]] > depth[v]){
                v = nodes[i];
            }
        }
    }
    ...
    for(int node : nodes){
        bool branch = is_ancestor(node, u) || is_ancestor(node, v);
        bool ceil = depth[node] >= depth[lca];
