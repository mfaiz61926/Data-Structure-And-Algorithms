...
void query(int u, int v){
    ...
    for(int i = MX_BITS - 1; i >= 0; i--){
        if(up[u][i] != up[v][i]){
            ans_mn = min({ans_mn, mn[u][i], mn[v][i]});
            ans_mx = max({ans_mx, mx[u][i], mx[v][i]});
    ...
    //last step(3) to the lca
    ans_mn = min({ans_mn, mn[u][0], mn[v][0]});
    ans_mx = max({ans_mx, mx[u][0], mx[v][0]});
    cout << ans_mn << " " << ans_mx << endl;
}
...
void m_conq() {
        ...
        //explicitly correct the root's base state so it does not pollute queries
        mx[1][0] = -1e9;
