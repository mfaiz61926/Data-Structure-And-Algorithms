...
void m_conq() {
        ...
        while(cin >> cmd && cmd != "DONE"){
            if(cmd == "DIST"){
                ...
                cin >> a >> b;
                int lca = get_lca(a, b);
                cout << dist[a] + dist[b] - (2LL * dist[lca]) << endl;
            }
            else if(cmd == "KTH"){
                int a, b, k;
                cin >> a >> b >> k;
                ...
                //nodes on the ascending side
                int left_count = depth[a] - depth[lca] + 1;
                if(k <= left_count){
                    cout << get_kth_ancestor(a, k - 1) << endl;
                }
                else{
                    int total_nodes = depth[a] + depth[b] - (2LL *depth[lca]) + 1;
                    int jump_from_b = total_nodes - k;
                    cout << get_kth_ancestor(b, jump_from_b) << endl;
                }
            }
        }
