...
int solve(string &s, int idx, int tight,int prev, int flag){
    if(idx == s.size()) return !flag;
    
    if(dp[idx][tight][prev][flag] != -1) return dp[idx][tight][prev][flag];

    int limit = (tight == 1 ? s[idx] - '0' : 9);
    int ans = 0;

    for(int d = 0; d <= limit; d++){ 
        int ntight = tight && (d == s[idx] - '0');

        if(prev == 10){ // no previous digit yet
            if(d == 0){
                ans += solve(s, idx + 1, ntight, 10, flag);
            }
            else{
                ans += solve(s, idx + 1, ntight, d, flag);
            }
        }
        else{
            int nflag = flag | (d == prev);
            ans += solve(s, idx + 1, ntight, d, nflag);
        }
              
    }

    return dp[idx][tight][prev][flag] = ans;
