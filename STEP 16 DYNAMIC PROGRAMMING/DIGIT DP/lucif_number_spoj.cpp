...
int solve(string &s, int idx, int tight, int oSum, int eSum){
    if(idx == s.size()){
        if(isPrime(eSum - oSum)) return 1;
        else return 0;
    }
    if(dp[idx][tight][oSum][eSum] != -1) return dp[idx][tight][oSum][eSum];

    int limit = (tight == 1 ? s[idx] - '0' : 9);
    int ans = 0;
    int posFromRight = s.size() - idx;

    for(int i = 0; i <= limit; i++){
        if(posFromRight % 2 == 0){
            int updateESum = eSum + i;
            ans += solve(s, idx + 1, (tight & (i == s[idx] - '0')), oSum, updateESum);
        }
        else{
            int updateOSum = oSum + i;
            ans += solve(s, idx + 1, (tight & (i == s[idx] - '0')), updateOSum, eSum);
        }
        
    }

    return dp[idx][tight][oSum][eSum] = ans;
