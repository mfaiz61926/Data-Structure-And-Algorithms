...
//     for (int len = 1; len <= min(n - i, rem + 1); len++) {
//         mn = min(mn, a[i + len - 1]);
//         int cost = len - 1;

//         ans = min(ans, mn * len + f(i + len, rem - cost));
//     }
...
void m_conq() {
        ...
        for(int i = 1; i <= n; i++){
            for(int used = 0; used <= k ; used++){
                int mn = v[i];
                for(int len = 1; len <= min(i, k + 1); len++){
                    mn = min(mn, v[i - len + 1]);
                    int cost = len - 1;
                    if(used >= cost){
                        dp[i][used] = min(dp[i][used], dp[i - len][used - cost] + mn * len);
                    }
                }
            }
        }   
