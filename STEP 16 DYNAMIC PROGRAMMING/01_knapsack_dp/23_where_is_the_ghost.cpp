...
// dp[i][r] = number of ways after processing the first i elements.

// This means:

// dp[0] → 0 elements processed
// dp[1] → first element processed
// dp[2] → first two elements processed
// ...
// dp[n] → all elements processed
int lcm (int a, int b){
...
void m_conq() {
        ...
        for(int i = 0; i < n; i++){
            for(int r = 0; r < l; r++){
                ...
                //don't take
                dp[i + 1][r] = (dp[i + 1][r] + dp[i][r]) % MOD;

                //take v[i]
                int nr = (r * v[i]) % l;
                dp[i + 1][nr] = (dp[i + 1][nr] + dp[i][r]) % MOD;
            }
        }
