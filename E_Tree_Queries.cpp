...
bool is_ancestor(int u, int v){
    ...
    int landing_node = get_kth_ancestor(v, diff);
    return landing_node == u;
}
...
void m_conq() {
        ...
        while(m--){
            ...
            for(int i = 0; i < k; i++){
                cin >> query_nodes[i];
                query_nodes[i] = up[query_nodes[i]][0];
            }

            int deepest = query_nodes[0];
            for(int i = 0; i < k; i++){
                if(depth[query_nodes[i]] > depth[deepest]){
                    deepest = query_nodes[i];
                }
            }

            bool possible = true;
            for(int i = 0; i < k; i++){
                if(!is_ancestor(query_nodes[i], deepest)){
                    possible = false;
                    break;
                }
            }
